<?php 
declare(strict_types = 1);
include "../../src/bootstrap.php";

is_admin($session->session_role);

$success = $_GET['success'] ?? null;
$failure = $_GET['failure'] ?? null;

$members = $cms->getMember()->getAll();

$navigation  = $cms->getCategory()->getAll(); 

$section = "";
$title = "Admin Members";
$description = "Admin Members";

?>


<?php include APP_ROOT . "/public/includes/admin_header.php" ?>


<br><br><br><br>
<main class="container text-center" id="content">

    <section class="hero-section text-center">
        <div class="flag">
            <h1>Admin List of Members</h1>
            <h3>Welcome <?= $session_first_name . ' ' . $session_last_name ?></h3>
            <hr>
        </div>
    </section>

    <br>

<!-- Start List of Members -->


<div class="row justify-content-center">
    <div class="category-success-failure-messages col-lg-10">
        <?php if($success) {?> <div class="alert alert-success"><?= $success ?></div> <?php } ?>
        <?php if($failure) {?> <div class="alert alert-danger"><?= $failure ?></div> <?php } ?>
    </div>
    <br>

    <div class="card admin_members">
        <section>
            <div class="card-header text-bg-info">
                <h2>List of Members</h2>
            </div>
        </section>

        <section class="card-body">
            <p><a href="add_member.php" class="btn btn-success">Add New Member</a></p>

            <table class="table table-hover">
                <thead>
                    <tr>
                        <th scope="col">ID</th>
                        <th scope="col">Picture</th>
                        <th scope="col">Role</th>
                        <th scope="col">First Name</th>
                        <th scope="col">LastName</th>
                        <th scope="col">Email</th>
                        <th scope="col">Last Updated</th>
                        <!-- <th scope="col">View</th> -->
                        <th scope="col">Edit</th>
                        <th scope="col">Delete</th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ($members as $member) { ?>
                        <tr>
                            <td scope="row"><?= $member['id'] ?></td>
                            <td><img src="../uploads/<?= $member['picture'] ?? 'default_avatar.jpg' ?>" width="100px"></td>
                            <td><a href="edit_role.php?id=<?= $member['id'] ?>"><?= $member['role'] ?></a></td>
                            <td><?= $member['first_name'] ?></td>
                            <td><?= $member['last_name'] ?></td>
                            <td><?= $member['email'] ?></td>
                            <td><?= $member['joined'] ?></td>
                            <!-- <td><a href="../member.php?id=<?= $member['id'] ?>" class="btn btn-success">View</a> </td> -->
                            <td><a href="edit_profile.php?id=<?= $member['id'] ?>" class="btn btn-warning">Edit</a> </td>
                            <td><a href="delete_profile.php?id=<?= $member['id'] ?>" class="btn btn-danger">Delete</a> </td>
                        </tr>                            
                    <?php } ?>
                </tbody>
            </table>
        </section>
    </div>
</div>
</main>
<br><br>
<!-- End List of Members -->

    

<?php include APP_ROOT . "/public/includes/admin_footer.php" ?>


